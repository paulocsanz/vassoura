...
pub fn build(
    ...
    for c in cands {
        ...
        items.push(PlanItem { cand: c, age_days: age, hint });
    }
    (items, rejected)
}
...
/// Empacota elegíveis em ordem LRU até a meta de espaço livre
/// (ou até `top` itens — meta pode ficar de fora, e isso é dito).
pub fn pack(mut items: Vec<PlanItem>, free: u64, until_free_gib: f64, top: usize) -> Packed {
    items.sort_by(|a, b| {
        b.age_days
